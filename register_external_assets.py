...
def main() -> None:
    ...
    root = Path.cwd()
    payload = build_external_asset_manifest(args.source, args.dataset, asset_root=args.asset_root)
    ...
    print(
        json.dumps(
            {"out": str(out.resolve()), "records": len(payload["records"]), "corpus_pages": len(payload["corpus_pages"])},
            indent=2,
        )
    )
